...
2. **Uso de la función en `usuarios_router.py`**:
   ...
   def post(self):
      ...
      if (
         UsuarioModel.query.filter_by(username=data["username"]).first()
         or UsuarioModel.query.filter_by(email=data["email"]).first()
      ):
         return {
               "message": "Usuario o correo electrónico ya registrado"
         }, 400
